...
export const seedManualTable = async ({ _db = db, data, tableName }: { _db?: PrismaClient; data: any[]; tableName: string }) => {
...
export const getColumnTypes = async (tableName: string): Promise<Record<string, string>> => {
  const res: { column_name: string; data_type: string }[] = await db.$queryRawUnsafe(`
